...
int main (int argc, char *argv[]) {
	...
	FILE *keypipe = popen("open src/index.html", "r");
	if (!keypipe) std::cerr << "Could not open js file." << std::endl;
	...
	if (argc == 2) {
		...
		std::cerr << "No patch given." << std::endl;
		return 1;
	}
	...
	while(1) {
		if (fgets(buffer.data(), 10, keypipe) != NULL) {
			strtok(buffer.data(), "\n ");
			key = buffer.data();
